...
option_list <- list(
    ...
    make_option(c("-n", "--nfold"), type="integer", default=5,
                ...
                metavar="character"),
    make_option(c("-z", "--inner_nfold"), type="integer", default=5,
...
if(opt$remove_missing_in_phenotype) {
  logging::loginfo('--remove_missing_in_phenotype is effective')
}
...
for(pheno in colnames(df_phenotype)[c(-1, -2)]) {
  ...
  message('All values for phenotype ', pheno, ' before checking missingness: ', mynsample)
  if(opt$remove_missing_in_phenotype) {
    ...
    mynsample = length(myy)
    message('Values retained for ', pheno, ' after removing missing: ', mynsample)
    if(mode == 'cv_performance') {
  ...
  for(k in 1 : opt$nfold) {
    ...
    # build refit col
    split_col2 = split_col
    split_col2[split_col2 == 'val'] = 'train'
    split_col2[split_col2 == 'test'] = 'val'

    # prep partitions for merge
    split_string <- strsplit(myx, "_")
    ss <- data.frame(FID = sapply(split_string, "[", 1), IID = sapply(split_string, "[", 2), 
                     split = split_col, split_refit = split_col2)
    
    # merge partitions
    print(dim(df_phenotype))
    if("split" %in% colnames(df_phenotype)){
      df_phenotype = df_phenotype[, -which(names(df_phenotype) == "split_refit")]
      df_phenotype = df_phenotype[, -which(names(df_phenotype) == "split")]
      df_phenotype = merge(df_phenotype, ss, by = c("FID", "IID"), all.x = TRUE)
    } else {
      df_phenotype = merge(df_phenotype, ss, by = c("FID", "IID"), all.x = TRUE)
    }
